...
class MainScrot:
    ...
    def __init__(self,saveFileName=None, saveFiletype="png"):
        ...
        self.window.set_icon_from_file(FileCatalog.get_deepin_scrot_icon())
    ...
    def initToolbar(self):
        ...
        self.actionSaveEverNoteButton = self.createOtherButton("saveEverNote", __("Tip save"))
        self.actionSaveEverNoteButton.connect("button-press-event", lambda w, e: self.saveSnapshotToEverNote())
    ...
    def saveSnapshotToEverNote(self):
        path = MetaData.get_note_buffer_path_name()
        filename = "%s%s.%s" % (DEFAULT_FILENAME, getFormatTime(), 'png')
        self.saveSnapshot(os.path.join(path,filename), 'png')
        logging.info('Picture saved to path %s/%s', (path,filename))
    ...
    def saveSnapshot(self, filename=None, filetype='png'):
        ...
        self.destroy(self.window)
         ...
         # tipWindow
        #cmd = ('python', root.get_full_path('src','tipswindow.py'), tipContent)
